...
const storySchema = new mongoose.Schema(
  {
    ...
    author: {
      // One to One: Each story has one author
      type: mongoose.Schema.Types.ObjectId,
      ref: "Author",
    ...
    characters: [
      {
        // One to few relation (embedded subdocuments)
        name: String,
        role: String,
        age: Number,
    ...
    likes: [
      {
        // One to Squillions: Massive number of likes (array of user IDs)
        type: mongoose.Schema.Types.ObjectId,
        ref: "User",
      },
    ],
...
router.put("/stories/:id", async (req, res) => {
  try {
    ...
    const updatedStory = await Story.findByIdAndUpdate(
      req.params.id,
      { title, subtitle, story, category: categoryDoc._id, tags: tagDocs },
      { new: true },
    );
    if (!updatedStory) {
      return res.status(404).json({ message: "Story not found" });
...
```js
mongoose.connect(
  "mongodb+srv://<username>:<password>@cluster0.mongodb.net/your-database",
);
```
